// Cubiomes Rebuild - Noise Generation (1.21+ only)
// Rebuilt from original cubiomes, keeping only functions used by 1.18+
...
double samplePerlin(const PerlinNoise *restrict noise, double d1, double d2, double d3,
        double yamp, double ymin)
{
    ...
    const uint8_t *restrict idx = noise->d;

    // Prefetch permutation table for upcoming lookups
    PREFETCH(idx + h1, 0, 3);
    PREFETCH(idx + h1 + 1, 0, 3);
...
{
    // MD5 hashes for octave initialization (1.18+)
...
double sampleOctave(const OctaveNoise *restrict noise, double x, double y, double z)
{
    double v = 0;
    int i;
    for (i = 0; i < noise->octcnt; i++)
    {
        const PerlinNoise *restrict p = noise->octaves + i;
...
double sampleOctaveAmp(const OctaveNoise *restrict noise, double x, double y, double z,
...
double sampleOctave2D(const OctaveNoise *restrict noise, double x, double z)
{
    double v = 0;
    int i;
    for (i = 0; i < noise->octcnt; i++)
...
//==============================================================================
// Double Perlin Noise (Used by 1.18+)
//==============================================================================
...
double sampleDoublePerlin(const DoublePerlinNoise *restrict noise,
